...
async def run_browser_agent(payload, websocket, browser, browser_context):
    ...
    if CookieManagerClass and browser_context:
        try:
            ...
            await cookie_manager.load_cookies(browser_context)
            await websocket.send_json({"type": "log", "content": "Cookies loaded into browser context."})
    ...
    agent = BrowserUseAgent(
        version="0.0.2",
        state=None,
        settings=agent_settings_prepared,
        task=task,
        llm=llm,
