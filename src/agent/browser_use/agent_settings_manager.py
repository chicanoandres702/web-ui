...
def sync_dependencies():
    ...
    Handles the tricky llama-cpp-python installation on Windows.
    """
    ...
    try:
        ...
        # 2. Playwright Binaries
        print("Installing Playwright browsers (Chromium)...")
        subprocess.check_call([sys.executable, "-m", "playwright", "install", "chromium"])
        
        # 3. Local LLM (LlamaCPP) - Windows Optimization
        if os.name == 'nt':
            print("Detected Windows: Attempting pre-built wheel for llama-cpp-python...")
            try:
                ...
                print("⚠️ Local Llama-CPP binary skipped. The app will default to Ollama API or Gemini.")
        else:
...
class Settings(BaseSettings):
    ...
    API_SECRET_KEY: str = "scholar-nexus-secure-key"
    GOOGLE_SERVICE_ACCOUNT_JSON: str = "service_account.json"
    
    # --- LLM CONFIG ---
    LLM_PROVIDER: str = "ollama" # Options: ollama, gemini, local_llama
    MODEL_NAME: str = "llama3" 
    OLLAMA_BASE_URL: str = "http://localhost:11434"
    GEMINI_API_KEY: str = "" # Set via .env or input
...
class LLMFactory:
    ...
    def create_llm(model_name: str = None):
        ...
        # Explicitly check for gemini models to use the Google provider
        if (m and m.startswith("gemini")) or p == "gemini":
            # If no API key is set in config, we hope it's in env vars, 
            # otherwise this might fail gracefully in the Orchestrator
            return ChatGoogleGenerativeAI(
                model=m if m and m.startswith("gemini") else "gemini-1.5-flash",
...
class BrowserAgentWrapper:
    ...
    async def decompose_task(self, task: str, class_name: str):
        prompt = f"Role: Research Assistant. Break academic task '{task}' for {class_name} into 3 steps. Return ONLY a raw JSON list of strings."
        try:
            # Invocation handles both Ollama and Gemini objects
            res = self.llm.invoke([HumanMessage(content=prompt)]).content
            # Basic cleaning for JSON parsing
            start = res.find('[')
            end = res.rfind(']') + 1
            if start == -1 or end == 0: raise ValueError("No JSON list found")
    ...
    async def run_step(self, description: str, class_name: str, headless: bool = False, quality: int = 50, callback=None) -> bool:
        if not HAS_LIBS: 
            if callback: await callback("log", "Browser libs missing. Skipping.")
            return True
        ...
        try:
            ...
            if callback: await callback("log", f"Browser Error: {str(e)}")
...
class TaskOrchestrator:
    ...
    async def _process_lifecycle(self, tid, req):
        ...
        try:
            ...
            state.error = f"LLM Init Failed: {e}"
            if cb: await cb("log", f"Error: {e}")
            return
        ...
        if req.require_confirmation:
            ...
            if state.status == TaskStatus.STOPPED:
                if cb: await cb("log", "Task stopped by user.")
                return
...
async def get_models():
    # Direct REST call to Ollama to populate dropdown
    try:
...
async def run_task(req: AgentTaskRequest):
    # Set the global websocket callback
...
# Windows SelectorEventLoop does not support subprocesses (required by Playwright).
# We must enforce ProactorEventLoop policy globally.
...
# Mount static files to root (fixes blank page)
...
if __name__ == "__main__":
    # Ensure Windows policy is set before server start
    if sys.platform == 'win32':
        asyncio.set_event_loop_policy(asyncio.WindowsProactorEventLoopPolicy())
...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; font-family: 'Inter', sans-serif; color: var(--txt); }
        ...
        input, select { width: 100%; background: var(--input); border: 1px solid var(--border); color: #fff; padding: 10px; border-radius: 6px; font-size: 13px; }
        label { font-size: 10px; opacity: 0.5; text-transform: uppercase; margin-bottom: -10px; display: block; }
        button { background: var(--acc); color: white; border: none; padding: 12px; border-radius: 6px; cursor: pointer; font-weight: 600; transition: filter 0.2s; }
...
<body>
    <div class="shell">
        <aside id="left">
            <h1 style="font-size:1.1rem; margin:0 0 1rem 0;">Scholar Pro</h1>
    ...
    <script>
        ...
        window.onGoogleAuth = (r) => {
            ...
            // Switch to Gemini mode automatically
            document.getElementById('provider-mode').value = 'gemini';
            refreshModelList();
        };
        async function refreshModelList() {
            ...
            if (mode === 'ollama') {
                ...
                if (googleUser) {
                    select.innerHTML = '<option value="gemini-flash-latest">gemini-flash-latest</option><option value="gemini-1.5-flash">gemini-1.5-flash</option>';
                } else {
        ...
        async function init() {
            ...
            document.getElementById('run-btn').onclick = async () => {
                ...
                // Don't set taskId here, wait for socket event
            };
