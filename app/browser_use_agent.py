...
class ControlQueueHandler:
    """Handles control commands (pause, resume, stop) from the user."""

    def __init__(self, agent: BrowserUseAgent):
        self.agent = agent

    async def _process_control_queue(self):
        """Processes commands from the agent control queue."""
        if not self.agent.agent_control_queue:
            return

        try:
            command_data = await asyncio.wait_for(self.agent.agent_control_queue.get(), timeout=0.1) # type: ignore
            command = command_data.get("command") # type: ignore

            if command == "pause":
                self.agent.pause()
                logger.info("Agent paused via user request.")
                if self.agent.send_agent_message_callback:
                    await self.agent.send_agent_message_callback({"type": "agent_status", "status": "Paused ⏸️"})
            elif command == "resume":
                self.agent.resume()
                logger.info("Agent resumed via user request.")
                if self.agent.send_agent_message_callback:
                    await self.agent.send_agent_message_callback({"type": "agent_status", "status": "Participating"})
            elif command == "stop":
                self.agent.stop()
class TabManager:
    """Manages browser tabs, closing extraneous ones."""

    def __init__(self, browser_context: Any):
        self.browser_context = browser_context

    async def manage_tabs(self):
        """Closes extraneous tabs to keep the agent focused."""
        nav_controller = getattr(self.browser_context, 'nav_controller', None)
        if nav_controller:
            await nav_controller.manage_tabs(self.browser_context)


class HeuristicsManager:
    """Manages heuristics for completion, progress, loops, and alternative strategies."""

    def __init__(self, agent: BrowserUseAgent):
        self.agent = agent
        self.is_waiting_for_mfa = False # type: ignore

    async def check_completion_heuristics(self):
        """Runs multiple completion heuristics to determine if agent has suceeded or failed to make adequate progress."""
        
        # loop_detection_handler = LoopDetectionHandler(self)
        # await loop_detection_handler.detect_loop()

        # progress_indicator_handler = ProgressIndicatorHandler(self)
        # await progress_indicator_handler.detect_progress()

        # # Check for progress indicators
        # blocking_element_handler = BlockingElementHandler(self)
        # await blocking_element_handler.check_blocking_elements()

        # navigation_recovery_handler = NavigationRecoveryHandler(self)
        # await navigation_recovery_handler.check_navigation_recovery()
        # # Check navigation recovery

        # subtask_addition_handler = SubtaskAdditionHandler(self)
        # await subtask_addition_handler.check_and_add_subtasks()

        # login_status_handler = LoginStatusHandler(self)
        # await login_status_handler.check_login_status()
        pass
    def detect_loop(self):
        """Detects if the agent is stuck in a loop and injects a message."""
        # TODO: Add actual implementation
        pass

    async def detect_progress(self):
        """Detects if the agent is making progress and injects a message."""
        # TODO: Add actual implementation
        pass

    async def check_blocking_elements(self):
        """Detects if there are blocking elements and injects a message."""
        # TODO: Add actual implementation
        pass

    async def check_navigation_recovery(self):
        """Detects navigation recovery and injects a message."""
        # TODO: Add actual implementation
        pass

    async def check_and_add_subtasks(self):
        """Checks for complex page structure and adds subtasks."""
        # TODO: Add actual implementation
        pass

    async def check_login_status(self):
        """Checks the login status and injects a message."""
        # TODO: Add actual implementation
        pass
    ...
    def inject_message(self, content: str):
        ...
        try:
            ...
            add_msg_func = getattr(self.agent.message_manager, "add_message", None)
            # print(f"trying add messafe", add_msg_func)
            if callable(add_msg_func):
                add_msg_func(msg)
            else:
                add_new_task_func = getattr(self.agent.message_manager, "add_new_task", None)
...
class BrowserUseAgent(Agent):
    def __init__(
        ...
        **kwargs
        ...
        self.heuristics = HeuristicsManager(self)
    ...
    def _initialize_internal_components(self, agent_kwargs, system_prompt):
        ...
        self.tab_manager = TabManager(self.browser_context)
    ...
    async def _execute_agent_step(self, step_info: AgentStepInfo):
        ...
        try:
            await self.step(step_info) # type: ignore
        except Exception as e:
    ...
    # Suggest alternative strategies on failure
        await self.tab_manager.manage_tabs()
