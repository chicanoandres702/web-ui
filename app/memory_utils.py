...
import asyncio
from app.utils.utils import save_text_to_file, read_file_safe, sanitize_filename
from app.config import get_config
logger = logging.getLogger(__name__)

async def _create_embedding(text: str, llm: Any = None):
    """
    ...
    try:
        if llm:
            ...
            if hasattr(llm, "aembed_query") and asyncio.iscoroutinefunction(llm.aembed_query):
                return await llm.aembed_query(text)
            # Fallback to sync method in a thread
            elif hasattr(llm, "embed_query") and callable(llm.embed_query):
...
class SimpleMemoryManager:
    ...
    """
    def __init__(self, memory_dir: str = get_config("MEMORY_DIR", "./tmp/memory")):
...
def get_memory_manager(memory_dir: str = get_config("MEMORY_DIR", "./tmp/memory")):
...
def configure_mem0():
    """Deprecated: No-op to prevent mem0 errors. Initializes backup manager if needed."""
    global _memory_manager
    if not _memory_manager:
